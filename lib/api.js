...
/**
 * Prisma connection-level failures.
 *
 * These mean the database is unreachable or refusing us — not that anything is
 * wrong with the request. They must be reported as such, because the first
 * thing almost every route does is read from the database, and reporting a
 * connection outage as an auth problem sends people hunting in the wrong place.
 */
const DATABASE_ERROR_CODES = new Set([
  ...
  'P1010', // access denied
  'P1013', // malformed connection string
...
function databaseErrorResponse(error, log) {
  log.error('api.database_unavailable', { code: error?.code, error });
  return apiError('CatalogPilot cannot reach its database right now. Please try again shortly.', {
    status: 503,
...
export function withAuth(handler) {
  return async function authedRoute(request, routeContext) {
    ...
    try {
      ...
      // A database outage surfaces here because authentication reads the
      // stored session. It is not an authentication failure.
      if (isDatabaseUnavailable(error)) return databaseErrorResponse(error, logger);
...
function handleRouteError(error, log, request) {
  ...
  if (isDatabaseUnavailable(error)) return databaseErrorResponse(error, log);
